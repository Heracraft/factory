...
const Usage = `repose-admin <command> [args]
  ...
  secrets   rewrap
  billing   rollup [--hour 2026-09-17T14] | resync --user HANDLE
...
func (e *Env) engine(ctx context.Context) (*ops.Engine, error) {
	...
	e.eng = ops.New(e.pool, nil, nil, nil, nil, metrics.NewNop(), obs.NewLogger(obs.LogOptions{Component: obs.ComponentAdmin, Writer: e.Stderr, Level: slog.LevelError + 1}), ops.Config{})
	return e.eng, nil
}
