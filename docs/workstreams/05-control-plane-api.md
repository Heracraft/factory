...
- `cmd/api/main.go`, `cmd/factory-admin/main.go`, and `internal/api/`
...
- `factory-admin`: `hosts add` (mints a join token), `hosts list|drain|
...
equals `https://api.factory.herakraft.co` and `exp`. The `sub` claim is the
...
`internal/api/ca`: two ed25519 keys generated by `factory-admin ca init`
...
workstream). Gateway host certificate: `factory-admin ca sign-host
--principal ssh.factory.herakraft.co` prints it for workstream 06's
deployment.
...
DEK, wrap it with Key Vault (`WrapKey` on the key `factory-dek-kek`, RSA-OAEP-256)
...
Base bumps: `factory-admin base release <version> --nix-rev <rev>
...
`factory-admin ca sign-client --name gateway`). `route` resolves
...
(it runs inside the Coolify network, `factory-admin` is an exec into the
...
Migrations run by a Coolify pre-deploy command `factory-admin db migrate`
...
in `ops/OBSERVABILITY.md`. Metrics prefix `factory_api_`: HTTP request
...
against the new schema. `factory-admin db migrate --down 1` exists and is
...
- [ ] `factory-admin` subcommands all exist and `hosts add` produces a
...
- [ ] `factory-admin db migrate --down 1` works in CI. Evidence: CI job.
- [ ] No `TODO`, `FIXME`, `panic(` outside main, `_ = err` under `cmd/api`,
      `cmd/factory-admin`, `internal/api`, `internal/db`. Evidence: grep.
