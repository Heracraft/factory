...
`factory` is the one static Go binary a developer installs. It turns "I am in
...
- `cmd/factory` and `internal/cli/` (Go 1.23+, cobra for commands, no
...
- Project resolution from the git remote with normalisation and the
  `projects.json` cache.
- SSH certificate issue and silent refresh, `~/.ssh/factory/` files, the one
...
```
factory login [--no-browser]
factory logout
factory run [PROMPT] [--agent claude|opencode|codex|gemini|pi] [--size small|large|xl]
            [--name NAME] [--stash-remote | --discard-remote] [--no-sync] [--no-attach]
factory attach
factory start
factory stop [--no-snapshot]
factory status [--json] [--watch]
factory open PORT [--local-port N] [--no-browser]
factory open --desktop [--no-browser]
factory secrets set NAME [--from-file PATH] [--from-env]
factory secrets list
factory secrets rm NAME
factory config show [--revisions]
factory config edit
factory config apply [PATH]      # PATH defaults to ./factory.nix if present, else opens editor
factory snapshots list
factory snapshots create
factory snapshots restore SNAPSHOT_ID [--as-new NAME]
factory destroy [--yes]
factory logs [--kind console|build|ops] [--since 1h] [--follow]
factory projects                  # list all, ignores cwd
factory version
factory completion bash|zsh|fish
factory mcp forward ...           # reserved, prints not-available message
factory browser bridge            # reserved, prints not-available message
```

Global flags: `--project ID|SLUG` (or `FACTORY_PROJECT`), `--api-url` (or
`FACTORY_API_URL`), `--json` on read commands, `-v` for debug logging to
stderr.
...
1. Discover Logto's OIDC config from `config.toml` `logto_issuer` (default
   ...
   for 24 hours.
2. If a browser is available (not `--no-browser`, `FACTORY_NO_BROWSER`
   unset, `DISPLAY` or macOS, not inside a guest (`FACTORY=1`)): start a
   listener on `127.0.0.1:0`, build the authorization URL with
   `code_challenge` (S256), `scope=openid offline_access profile email`,
   `resource=https://api.factory.herakraft.co`, open the browser, wait up to
...
4. Store per `interfaces/cli-config.md`: macOS keychain for the refresh
   token (service `factory`, account `<issuer>`), else
   `credentials.json` 0600.
5. Call `GET /me`. If `billing.has_card` is false, print

   ```
   No card on file. Add one at https://factory.herakraft.co/billing before
   the first `factory run`.
   ```
...
with `Not logged in. Run \`factory login\`.`

`factory logout` revokes the refresh token at Logto, deletes the stored
tokens, calls `POST /certs/revoke {all:true}`, removes
`~/.ssh/factory/id_ed25519-cert.pub`. It leaves `projects.json` and the ssh
config in place.
...
1. `--project` / `FACTORY_PROJECT`: id or slug, resolved via `GET
...
4. Nothing found and the command is `run`: create (see 5.5). Nothing found
   ...
   ```
   No factory project for github.com/a/b. Run `factory run` here to create one,
   or pass --project.
   ```
...
- Read `~/.ssh/factory/id_ed25519-cert.pub`; if present, valid for more
...
- Else ensure `~/.ssh/id_ed25519` exists (`ssh-keygen -t ed25519 -N ""` with
  a comment `factory` if missing; never touch an existing key), `POST /certs`
...
- Write `~/.ssh/factory/known_hosts` with `@cert-authority
  ssh.factory.herakraft.co,10.64.* <host_ca_pub>` from the response.
- Rewrite `~/.ssh/factory/config` with one `Host <slug>.factory` block per
  project (the block in `interfaces/ssh-gateway.md`). Ensure `~/.ssh/config`
  contains `Include ~/.ssh/factory/config` as its first line, inserted once,
  with a comment `# added by factory`. Never rewrite any other line.
...
3. `ensureCert`.
4. Wait until `ssh <slug>.factory true` succeeds, up to 60 seconds after the
...
5. Sync (unless `--no-sync`):
   ...
   b. Remote, in one SSH exec: `cd ~/<slug> && git status --porcelain`. If
      ...
      `git stash`) or --discard-remote (throws them away), or `factory attach`
      to look first.
      ```

      `--stash-remote` runs `git stash push -u -m "factory run <ts>"`,
...
7. If PROMPT given: agent = `--agent` or project `agent_default`. Over SSH:
   ...
   ```
8. Unless `--no-attach`: `exec ssh -t <slug>.factory tmux attach -t <slug>`
...
`factory attach` is steps 1 (resolve, no create), 3, 4, 8.

### 5.6 stop, start, destroy, resize

- `stop`: `POST /stop {snapshot: !--no-snapshot}`, spinner on the op, then
  `Stopped <slug>. Snapshot <id> (1.2 GB). Disk is still billed; \`factory
...
- `destroy`: prints the project, its last snapshot date, and requires typing
  ...
  Last snapshot kept until <date>; \`factory snapshots restore <id> --as-new
  NAME\` brings it back.`
- Resize is `factory config apply` with `volume_bytes` in the fragment
  header? No: it is its own route, so `factory resize 80G` exists as a
...
`factory projects` prints one line per project in the same first-line
format.
...
`factory open 3000`: `ensureCert`, then `ssh -N -L
127.0.0.1:<local>:127.0.0.1:3000 <slug>.factory` in the foreground, print
...
`factory open --desktop`: over SSH `systemctl --user start factory-desktop`
...
- `config edit`: fetches, opens `$EDITOR` on a temp file, on save `PUT` and
  ...
  run \`factory stop && factory start\` when the agent is idle.`
- `config apply [PATH]`: same with a file; `./factory.nix` default. The file
...
`install.sh` at `https://factory.herakraft.co/install.sh`: detects
`uname -sm`, downloads `factory_<version>_<os>_<arch>.tar.gz` from the
...
github:heracraft/factory#factory` works via the flake's `packages.factory`.
...
| Situation | Outcome |
|---|---|
| Not logged in or refresh failed | exit 3, `Not logged in. Run \`factory login\`.` |
| No card on file on create/start | exit 7, `Add a card at https://factory.herakraft.co/billing first.` |
| `capacity` from create/start | exit 8, `No capacity right now; try again in a few minutes. (We have been alerted.)` |
| Guest stopped on `attach`/`open` | exit 5, `todo-app is stopped. Run \`factory start\`.` |
| Dirty remote tree | exit 6, message in 5.5 |
| Build or eval error | exit 10, Nix error block, fragment line marked |
| SSH cannot connect within 60 s after `running` | exit 1, `Guest is running but SSH did not answer in 60s. \`factory logs --kind console\` may show why.` |
...
| API unreachable | exit 1, `Cannot reach api.factory.herakraft.co: <err>`; never retried more than 3 times with backoff |
| Rate limited on `/certs` | reuse existing cert if valid, warn on stderr |
| Prompt given but agent not installed in guest | exit 1, `Agent 'pi' is not in this guest's config. Add it with \`factory config edit\`.` |
...
only files it owns on a laptop are under `~/.config/factory/` and
`~/.ssh/factory/` plus the one `Include` line; `factory logout --purge`
removes all of them including the `Include` line.

## 9. Checklist

- [ ] Every command and flag in 5.1 exists with that name. Evidence: `factory
...
- [ ] `~/.ssh/config` gets exactly one `Include` line, first line, and no
      ...
      pre-existing config.
- [ ] `ssh <slug>.factory` works from a plain terminal with no CLI involved
      after one `factory run`. Evidence: transcript.
...
- [ ] `attach` execs ssh (the CLI process is replaced). Evidence: `ps`
      shows no `factory` parent during a session.
...
- [ ] `--json` output on every read command is valid JSON with nothing else
      on stdout. Evidence: `factory status --json | jq .` in CI.
- [ ] GoReleaser builds four targets; `install.sh` installs on ubuntu and
      macos CI runners and `factory version` prints the tag. Evidence: CI
      run link.
- [ ] `nix run .#factory` works. Evidence: CI job.
