...
| Guest cannot reach host | `guest_in` drop (ICMP echo excepted at 5/s, DECISIONS I-18; replies to flows the host itself opened, I-70, which no packet a guest sends first can match) | from A: connect to host `.1` on 22, 9101, 8080 fails; a `ping` flood of `.1` loses most packets |
