...
- `nix/guest/base/ssh.nix`: openssh with the settings in
  ...
  path (`/run/repose/secrets/ssh_host_ed25519_key`, symlinked into
  `/etc/ssh` by an activation script); `TrustedUserCAKeys` read from
  `/run/repose/user_ca.pub`; `AuthorizedPrincipalsFile
...
- `nix/guest/microvm.nix`: a function `mkGuestRunner { fragmentModule,
  ...
  [{ tag = "ro-store"; source = "/run/repose/store-export"; mountPoint =
  "/nix/.ro-store"; proto = "virtiofs"; socket = "/var/lib/repose/guests/
