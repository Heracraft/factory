...
`[{id, options: {name: value}}]`; the api validates it (an unknown id or
value is `invalid` naming it) and renders it into a fragment whose first
two lines are `# generated by repose from your menu selection; edit with
repose config edit to take over` and `# repose-menu: <the selection as
JSON>`, one `imports` module per entry. The api keeps a project in menu
mode while its fragment still starts with those lines.
