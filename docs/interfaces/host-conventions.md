...
| Path | What |
|---|---|
| `/var/lib/factory/hostd/` | `cert.pem`, `key.pem` (mTLS to api), `host.json` (host id, guest cidr, wg keys), `state.db` (bbolt: guest table for reconciliation) |
| `/var/lib/factory/guests/<guest_id>/` | `runner` (symlink to the microvm.nix runner package), `ch.sock` (Cloud Hypervisor API), `console.log`, `virtiofsd.sock`, `secrets/` (tmpfs mount, delivered to guest at boot then unmounted) |
| `/run/factory/join-token` | one-shot registration token from cloud-init, deleted after Register |
| `/nix/var/nix/gcroots/factory/<guest_id>` | GC root for the guest's system closure; removed on destroy |
| `/dev/vg-guests/thin` | thin pool; volumes `/dev/vg-guests/g-<guest_id>` |
| `/var/log/factory/` | hostd log (journald is primary), build logs per op |
...
- nftables table `factory`: chains `guest_fwd` (default drop; allow
...
`factory-snapshot.timer` (nightly 03:00 local).
...
to `audit_log` via a PAM hook that calls hostd. `factory-admin exec` is the
