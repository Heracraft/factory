...
| `/run/repose/secrets/<NAME>` | named secret values, tmpfs, 0400 dev |
| `/run/repose/secrets.env` | `export NAME='...'` lines, sourced by login shells |
| `/run/repose/hooks.sock` | hook ingest, HTTP over unix, group dev |
| `/run/repose/guestd.sock` | dev-only stand-in for vsock (absent in real guests) |
| `/etc/repose/base-version` | the platform base version string |
...
1. Ensure its hook config points at `/run/repose/hooks.sock` (Claude Code:
   `~/.claude/settings.json` hooks `Notification` and `Stop` run
   `repose-hook`; others per their doc in `features/agents.md`).
2. Set `TERM=tmux-256color`, `COLORTERM=truecolor`.
3. Exec the real binary.

`repose-hook` reads the hook JSON from stdin, maps it to `{agent, kind,
...
`REPOSE_PROJECT=<slug>`, `REPOSE=1` (so scripts can detect they are in a
