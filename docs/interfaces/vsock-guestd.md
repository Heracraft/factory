...
| `Sample` | | GuestSignals + repeated ProcSample (shapes in grpc-hostd.md) + partial (bool) | hostd calls every 60 s. guestd walks `/proc` on the call and serves the tmux and Docker signals from a 5 s background refresh, so a sample costs under 20 ms and never forks (DECISIONS I-31). `partial` is set when a signal is missing rather than zero. `signals.listening` (I-207) comes from the same 5 s refresh: `/proc/net/tcp` and `tcp6` for the listening sockets, each process's fd links matched against their inodes (`socket:[N]` only), and the owner's `stat` for comm, start time and RSS. The same refresh sets `oom_score_adj` (I-200): -800 for the tmux server and each agent window's agent process, 0 for any other `dev` process holding a negative value |
