...
| `SetupProject` | project_slug, remote_url, tz, lang | | creates tmux session named slug, `/home/dev/<slug>` (a symlink to the previous slug's checkout on a volume restored under a new name, I-255), git init if empty, writes `/home/dev/.repose/project.json` |
