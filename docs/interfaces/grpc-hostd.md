...
| `Restore` | project_id, guest_id (new), blob_path, class, volume_bytes, plus every CreateGuest field (secrets, env, ssh_ca_pub, principals, hooks_config, host_key, host_cert) | as CreateGuest |
| `UpdateSecrets` | guest_id, secrets | |
| `SetPrincipals` | guest_id, principals | |
| `Exec` | guest_id, argv, timeout_s (operator use only, audited) | exit_code, stdout, stderr (each capped 64 KB) |
| `Drain` | (host) | |

**Result**: `command_id`, `ok`, `error {code, message}`, payload. Error codes:
...
same command_id and hostd returns the stored result or re-executes if it
never started.
