...
|---|---|---|
| `CreateGuest` | project_id, guest_id, class, volume_bytes, system_closure (store path of the built system), secrets (list of name → bytes, delivered to tmpfs), env (TZ, LANG), ssh_ca_pub, principals (list), hooks_config, host_key (guest sshd ed25519 private key, generated by the api), host_cert (its certificate signed by the Host CA with principals guest_ip and `<slug>.<handle>`), user_id, project_slug, remote_url, project_json (I-18) | guest_ip, vsock_cid |
| `StartGuest` | guest_id, plus optionally the same delivery fields as CreateGuest: secrets, env, ssh_ca_pub, principals, hooks_config, host_key, host_cert, project_json (I-18: hostd keeps none of them on disk, so the api sends them on every start; a StartGuest with only guest_id is accepted while hostd still has them in memory) | |
| `StopGuest` | guest_id, snapshot_first (bool), timeout_s | snapshot_id if taken (assigned by the api), blob_path, bytes (I-18) |
