...
| POST | `/projects` | `{name, remote_url?, class, tz?, agent_default?}` → `Project` (`agent_default` defaults to `claude`; the CLI sends `config.toml`'s `default_agent`, I-227) (409 if `(user, remote_url)` or `(user, name)` exists) |
